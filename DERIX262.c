...
int getkey()
{
	...
	reg.x.ax = 0;
#if __GCCIA16__
	__asm__ __volatile__("movw %%ax, 0; int $0x16"
						 : "=a"(reg.x.ax));
#else
	int86(0x16, &reg, &reg);
#endif
...
#define RIX_FILENAME_MAX_LEN 20
#define PLAYLIST_MAX_LEN 999
FILE *fp = NULL;
FILE *tfp = NULL;
char rix_filenames[RIX_FILENAME_MAX_LEN][PLAYLIST_MAX_LEN];
char filename[RIX_FILENAME_MAX_LEN];
int num_tracks = 0;
int current_track = 0;
int counter = 0;
int num_max_repetition = 1;
int current_repetition = 1;
DWORD filelen = 0;
static DWORD I = 0;
BYTE rix_data[32767] = {0}; /* rix files' buffer */
WORD buffer[300] = {0};
WORD mus_block = 0;
WORD ins_block = 0;
...
main(int parmn, char *parms[])
{
	...
	memset(rix_filenames, 0, RIX_FILENAME_MAX_LEN * PLAYLIST_MAX_LEN);
	memset(filename, 0, RIX_FILENAME_MAX_LEN);

	/*-------------------------- Title ------------------------*/
	printf("DERIX262-With-Playlists Version 0.10 by srhouyu is a small update of\n");
	printf("\nDERIX262 Version 0.10 by SDLPal Team\n");
	...
	printf("\n[P]:Pause\n[C]:Continue\n[1] to [-]:Single Channel ON/OFF \n");
	/*
	printf("[A]: Previous Track\n");
	printf("[D]: Next Track\n");
	*/
	...
	if (stricmp(filename + (strlen(parms[1]) - 4), ".txt") == 0)
	{
		read_playlist(filename);
		if (parmn < 3)
			num_max_repetition = 1;
		else
			num_max_repetition = max(1, atoi(parms[2]));
	}
	else
	{
		strcpy(rix_filenames[0], filename);
		num_tracks = 1;
	}
	...
	{
		printf("\nUsage:\n");
		printf("\tPlayRix [FileName].Rix\n");
		printf("\tPlayRix [Playlist].txt [NumRepetitionsPerTrack]\n");
		exit(1);
	}

	strcpy(filename, rix_filenames[0]);
	...
	while (true)
	{
		...
		/*
		case next:		next_track(); play_track(); break;
		case prev:		prev_track(); play_track(); break;
		*/
...
/*                                                          */
/************************************************************/
void read_playlist(const char *txt_file_name)
{
	FILE *fp;
	char line[RIX_FILENAME_MAX_LEN];
	int len;	
	int i;
	fp = fopen(txt_file_name, "r");
	if (fp == NULL)
	{
		printf("Can not open playlist file %s", txt_file_name);
		exit(-1);
	}
	while (fgets(line, RIX_FILENAME_MAX_LEN, fp))
	{
		for (i = strlen(line) - 1; i >= 0; i--)
			if (line[i] == ' ' || line[i] == '\n')
				line[i] = '\0';
		strcpy(rix_filenames[num_tracks], line);
		num_tracks++;
		if (num_tracks >= PLAYLIST_MAX_LEN)
			break;
	}
}
/*----------------------------------------------------------*/
...
void next_track()
{
	...
	strcpy(filename, rix_filenames[current_track]);
}
...
void load_rix()
{
	...
	if (fp == NULL)
	{
		...
		music_ctrl(); 
		set_old_int(); 
		ad_bopw(OPL3_MODE_REGISTER, 0); 
		exit(-1);
	}
	fseek(fp, 0, SEEK_END);
	filelen = ftell(fp);
	if (filelen > 32767 || filelen < 0)
		filelen = 32767;
	fseek(fp, 0, SEEK_SET);
	fread(rix_data, 1, filelen, fp);
	fclose(fp);
}
/*----------------------------------------------------------*/
void rix_data_init()
{
	music_spd = mus_time;
	rix_stereo = rix_data[2];
	mus_block = (rix_data[0x0D] << 8) + rix_data[0x0C];
	ins_block = (rix_data[0x09] << 8) + rix_data[0x08];
	I = mus_block + 1;
	if (rix_stereo != 0)
	{
...
{
	word temp = 1;
	printf("Track%3d/%-3d ", current_track + 1, num_tracks);
	for (i = 0; i < 11; i++)
	{
		if (channel_on[i] == 1)
		{
...
word rix_proc()
{
	...
	if (music_on == 0 || pause_flag == 1)
		return 0;
	band = 0;
	while (rix_data[I] != 0x80 && I < filelen - 1)
	{
		band_low = rix_data[I - 1];
		ctrl = rix_data[I];
		I += 2;
		switch (ctrl & 0xF0)
		{
		case 0x90:
			...
			break;
		case 0xA0:
			rix_A0_pro(ctrl & 0x0F, ((word)band_low) << 6);
			break;
		case 0xB0:
	...
	music_on = 1;
	current_repetition ++;
	if (current_repetition > num_max_repetition){
		current_repetition = 1;
		next_track();
		play_track();
	}
...
void rix_get_ins()
{
	memcpy(insbuf, (&rix_data[ins_block]) + (band_low << 6), 56);
}
/*--------------------------------------------------------------*/
